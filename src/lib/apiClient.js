...
export function createApiClient(fetchImpl = globalThis.fetch) {
  ...
  return {
    ...
    actions: (options) => request('/api/actions', options),
    createAction: (payload, options) => request('/api/actions', { ...options, method: 'POST', body: payload }),
    updateAction: (actionId, payload, options) => request(`/api/actions/${encodeURIComponent(actionId)}`, { ...options, method: 'PATCH', body: payload }),
