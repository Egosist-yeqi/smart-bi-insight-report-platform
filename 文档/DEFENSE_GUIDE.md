...
本项目面向中小企业经营分析场景，将销售订单数据、指标监控、自然语言查询、经营报告、异常归因和趋势预测整合为一套可本地运行的 BI 平台。答辩时应强调：它不是只展示页面的原型，而是带有 Vue 3 应用入口、MySQL 8.4 LTS 数据库、FastAPI 后端、可选 PyTorch CPU 预测、可配置 AI 服务和自动化测试的完整系统。
...
| 时间 | 操作 | 要点 |
| --- | --- | --- |
| 0:00-0:35 | 打开场景库，确认“电商经营” | 展示行业语义、540 条演示数据、模板问题和可复现的数据来源。 |
| 0:35-1:35 | 打开仪表盘，选择“华东”后再清空筛选 | 展示 KPI、多维下钻、指标公式和数据范围。 |
| 1:35-2:25 | 在智能查询点击“本月各区域GMV / 交易额排名如何？” | 展示自然语言、受控意图、只读 SQL、图表、表格和 CSV。 |
| 2:25-3:15 | 打开异常归因和趋势预测 | 强调阈值预警、核查证据和预测边界。 |
| 3:15-4:05 | 在行动中心载入草案并创建行动 | 展示建议经过人工确认后才进入执行与复盘。 |
| 4:05-4:45 | 打开报告生成，生成月报并导出 Markdown | 展示报告模块化与可交付产物。 |
| 4:45-5:20 | 在场景库预检或导入模拟 CSV，查看数据版本 | 展示 11 字段边界、无副作用预检和 SHA-256 批次追溯。 |
| 5:20-5:45 | 打开系统配置 | 展示 DeepSeek 仅输入 Key、其他兼容服务完整配置入口。 |
| 5:45-6:00 | 总结安全边界与回退策略 | 强调 AI 只解析意图、SQL 只读校验、失败可回退。 |

详细逐字稿见 [演示讲稿](demo/演示讲稿.md)，输入问题清单见 [演示问题清单](demo/演示问题清单.md)。需要覆盖所有页面、行业切换与导入流程时，直接使用 [全功能演示素材库](demo/全功能演示素材库.md)；其中的 [业务角色任务卡](demo/业务角色任务卡.md) 可用于双人或分组演示。
