...
用户可在场景库下载对应 CSV 模板并导入自己的完整业务明细表。导入严格要求 11 个受控字段：记录编号、日期、组织、城市、项目编号、项目名称、类别、对象类型、数量、金额和收益；这让系统可以稳定提供指标、趋势、异常、归因线索、行动建议和预测，而不会对任意未知表结构编造结论。场景素材和字段模板见 [行业场景演示素材库](文档/demo/scenarios/README.md)。
