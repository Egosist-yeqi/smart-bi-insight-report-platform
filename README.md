...
- [系统功能演示文档](文档/DEMO_GUIDE.md)：逐步说明在什么位置输入什么内容。
- [演示素材包](文档/demo/README.md)：演示讲稿、问题清单和检查清单。
- [全功能演示素材库](文档/demo/全功能演示素材库.md)：12 分钟剧本、五行业案例、角色任务卡与可导入 CSV 样表。
...
- [测试与验收报告](文档/测试与验收报告.md)
- [项目 11 原始范围整理](文档/SOURCE_PROJECT_11.md)
- [开发过程文档](文档/PROCESS_2026-07-09.md)
- [演示素材包](文档/demo/README.md)
- [全功能演示素材库](文档/demo/全功能演示素材库.md)
- [答辩材料](文档/答辩材料/README.md)
- [官方格式项目最终文档](项目最终文档/README.md)：包含 1 至 11 号正式 Word/Excel 材料和实机截图素材库；第 12 号成员分工表按要求未编写。

## Windows 本地运行

系统使用 Docker Compose 运行 Vue 3 应用入口、FastAPI 后端和 MySQL 8.4 LTS。现有工作台页面以兼容视图形式保留，确保场景、查询、报告、异常、预测、行动、配置等功能不变。MySQL 不需要单独安装，端口只绑定到本机 `127.0.0.1:3307`，不会暴露到局域网。
...
> **MySQL 版本说明：** 原技术方向写为 MySQL 8.0；项目保留更新的 MySQL 8.4 LTS，维护性更好。不需要、也不会将现有数据卷降级。详见 [技术约束复查与修订说明](文档/技术约束复查与修订说明.md)。
...
用户可在场景库下载对应 CSV 模板并导入自己的完整业务明细表。上传后先点击“检查数据质量”，确认记录数、时间范围、维度覆盖、金额/收益汇总和限制提示，再点击“确认导入并替换当前数据”。导入严格要求 11 个受控字段：记录编号、日期、组织、城市、项目编号、项目名称、类别、对象类型、数量、金额和收益；这让系统可以稳定提供指标、趋势、异常、归因线索、行动建议和预测，而不会对任意未知表结构编造结论。仪表盘的“指标口径与数据范围”可随时核对结果的数据覆盖和公式。场景素材和字段模板见 [行业场景演示素材库](文档/demo/scenarios/README.md)。

每次实际加载演示数据或确认导入 CSV 后，场景库底部会记录来源、行业模板、记录数、日期范围和 SHA-256 数据指纹；“检查数据质量”始终是只读操作，不会替换数据或新增批次。详见 [数据版本追溯说明](文档/数据版本追溯说明.md)。

### 决策行动闭环

“行动中心”将异常、预测和查询的核查方向整理为与当前场景匹配的建议草案。用户确认后填写负责人、截止时间、目标指标和数据证据，创建待确认行动；开始执行时可标记为“执行中”，关闭时必须填写复盘结论。系统不会自动批准、执行或关闭行动，避免把分析建议误当作已经发生的业务决策。详细步骤见 [决策行动闭环说明](文档/决策行动闭环说明.md)。

### 答辩材料

- [答辩主文档：项目特色、架构、演示流程与问答](文档/DEFENSE_GUIDE.md)
- [三周 Codex 辅助开发交付说明](文档/THREE_WEEK_CODEX_DELIVERY.md)
